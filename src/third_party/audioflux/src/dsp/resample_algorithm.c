...
void resampleObj_setSamplate(ResampleObj resampleObj,int sourceRate,int targetRate){
	...
	// --- libspeech patch (see UPSTREAM_PATCHES.md) ---
	// Original code also early-returned when sourceRate==targetRate, which left
	// resampleObj->ratio at its hardcoded default of 0.5 (set in
	// resampleObj_newWithWindow), silently dropping half the samples on any
	// "identity" resample. Removing that clause lets the normal computation
	// below run: gcd(sourceRate, sourceRate) == sourceRate, so p=1, q=1,
	// ratio=1.0, which is the correct identity behaviour.
